...
uma_ret_code uma_device_list(Mooltipass_list *mooltipass_list) {
	...
	if(device_list_result < 0) {
		const char* err_message = libusb_strerror(device_list_result);
	...
	for(size_t i = 0; i < device_count; i++) {
		...
		mooltipass_list->devices = realloc(mooltipass_list->devices, mooltipass_list->count * sizeof(Mooltipass));
...
uma_ret_code uma_device_open_first(Mooltipass *mooltipass) {
	...
	Mooltipass_list mooltipass_devices = {0};
...
void uma_device_info_print(Mooltipass mooltipass) {
	...
	for(int i = 0; i < (int)config->bNumInterfaces; i++) {
		...
		for(size_t j = 0; j < interface->num_altsetting; j++) {
